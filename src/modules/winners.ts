...
import { sort, winnerData } from "./types";
...
class Winners {
    ...
    static winnersCount = async function() {
        ...
        Main.totalWinners.innerHTML = `${Math.ceil(Winners.totalWinnersCount / 10)}`;
        const WINNERS_HEADER = document.querySelector('.winners_and_count');
        if (WINNERS_HEADER) WINNERS_HEADER.innerHTML = `Winners (${Winners.totalWinnersCount})`;
    }

    static winnerCreator = async function (id: number, wins: number, time: number, count: number) {
        try {
            ...
            WIN_CARD_HTML.innerHTML = `
                                        ...
                                        <td>${time}</td>
                                        `;
            const container = document.querySelector('.winners_body');
            if (container) container.append(WIN_CARD_HTML);
            Winners.winnersCount();
        } catch {
            console.log('winner creation error');
        }
    }
    ...
    static addWin = async(id: number, time: number) => {
        try {
            const car = await Server.getWinner(id);
                ...
                console.log('winner add error');
            }
    }

    static sortWinners(sort: sort) {
        Winners.sort = sort;
        Winners.render();
    }
    ...
    static render = async () => {
        ...
        try {
            const winners: winnerData[] = await Server.getWinners([{key: '_page', value: `${Pagination.currentPageW}`}, {key: '_limit', value: `${10}`}, {key: '_sort', value: `${Winners.sort}`}]); 
            winners.forEach((el, count) => {
                if (el.id) Winners.winnerCreator(el.id, el.wins, el.time, count);
            });
        } catch {
